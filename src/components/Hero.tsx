...
import { mstats } from '@/data/makemoney';
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm mb-8">
          ...
          <span className="text-sm font-medium" style={{ color: 'rgba(255,255,255,0.8)' }}>
            {isZh ? `真实用户案例 · 收录 ${mstats.totalUsercases}+ 篇用户案例及教程` : `Real UserCases · ${mstats.totalUsercases}+ UserCases and Tutorials Curated`}
          </span>
        </div>
        ...
        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black mb-4 sm:mb-6 tracking-tight" style={{ color: '#fff' }}>
          {isZh ? (
            <>OpenClaw<span className="gradient-text"> Learn hub</span> </>
          ) : (
        ...
        <p className="text-base sm:text-lg md:text-xl font-semibold mb-3 sm:mb-4 px-2" style={{ color: 'rgba(255,255,255,0.9)' }}>
          {isZh ? '从零开始，5小时掌握OpenClaw AI 私人助理' : 'Your AI assistant that actually does things'}
        </p>

        {/* Secondary tagline */}
        <p className="text-xs sm:text-sm md:text-base mb-6 sm:mb-10 max-w-xl mx-auto px-3" style={{ color: 'rgba(255,255,255,0.5)' }}>
          {isZh 
            ? 'The open-source guide to building your AI assistant with OpenClaw in 5 hours — curated tutorials, learning paths, and real-world use cases to get you up and running fast.'
        ...
        <div className="mt-8 sm:mt-16 mx-auto max-w-md grid grid-cols-2 gap-y-4 gap-x-6 sm:hidden">
          ...
          <div className="text-center">
            <div className="text-xl font-bold" style={{ color: '#fff' }}>{isZh ? '5 小时' : '5 Hours'}</div>
            <div className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>{isZh ? '极速自学教程' : 'Rapid Self-Learning Course'}</div>
          </div>
          <div className="text-center">
        ...
        <div className="hidden sm:mt-16 sm:flex items-center justify-center gap-8 md:gap-12">
          ...
          <div className="text-center">
            <div className="text-2xl md:text-3xl font-bold" style={{ color: '#fff' }}>{isZh ? '5 小时' : '5 Hours'}</div>
            <div className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>{isZh ? '极速自学教程' : 'Rapid Self-Learning Course'}</div>
          </div>
          <div className="w-px h-8" style={{ background: 'rgba(255,255,255,0.1)' }} />
          <div className="text-center">
