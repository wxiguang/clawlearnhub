...
const texts = {
  en: {
    ...
    heroTitle: 'OpenClaw Business Hub',
    heroSubtitle: 'Gumroad, Patreon, Upwork, Shopify, YouTube, Bilibili…\nOne-stop access to OpenClaw profit ideas & OPC launch tutorials. Open source, continuously updated.',
    statsUsercases: 'tutorials',
    ...
    zhLabel: '中文',
    enLabel: 'EN',
    contributeTitle: '🤝 Found a great tutorials ?',
    contributeDesc: 'This is an open source project. Edit',
    contributeDesc2: ', submit a PR, and help more people.',
    submitPR: 'Submit PR',
    footerMadeBy: 'Made by Terry',
    catDescriptions: {
      'OPC': 'First-hand materials from the OpenClaw team',
      'getting-started': 'Beginner-friendly step-by-step tutorials',
      'Workflows': 'AI workflow setups',
      'AI Agents': 'Feishu, DingTalk, Telegram, WeChat Work integration',
  ...
  zh: {
    backToHome: '返回首页',
    heroTitle: '全网赚钱案例与教程',
    heroSubtitle: '阿里云、AWS、DigitalOcean、Youtube、Codecademy、IBM……\n一站式获取 OpenClaw 最佳教程，开源共享，持续更新。',
    statsUsercases: '篇教程',
    statsChinese: '中文',
    statsEnglish: '英文',
    statsCategories: '大分类',
    ...
    catDescriptions: {
      ...
      // 'AI Agents': '来自 OpenClaw 团队的第一手资料',
      // 'Workflows': '主流云平台一键部署方案',
      // 'getting-started': '从零开始的保姆级教程',
      // 'Case Study': '飞书、钉钉、Telegram、企业微信等平台接入',
      // 'OPC': '创建一人公司赚钱',
      // video: '看得见的教程更好学',
      // 'deep-dive': '架构分析、行业评论和进阶内容',
      // 'Passive Income': '自动化工作流和持续运营方案和辅助工具',
      // 'use-cases': '真实工作流、自动化方案和创意玩法',
    },
  },
};
...
function CategorySection({ 
  ...
  const items = usercases.filter((r) => r.category === cat);
...
/* ── page ── */
export default function UsercasesPage({ locale }: UsercasesPageProps) {
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-start gap-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-3 mt-8">
                {[
                  { n: mstats.totalUsercases + '+', l: t.statsUsercases },
                  { n: mstats.zhUsercases, l: t.statsChinese },
                  { n: mstats.enUsercases, l: t.statsEnglish },
                  { n: mstats.totalCategories, l: t.statsCategories },
                ].map((s) => (
            ...
            <div className="hidden lg:block relative w-80 h-72 shrink-0">
              ...
              <div className="absolute top-20 left-0 w-52 rounded-xl p-4 shadow-2xl transform -rotate-2 hover:rotate-0 transition-transform duration-500" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', backdropFilter: 'blur(12px)' }}>
                <div className="flex items-center gap-2.5 mb-2">
                  ...
                  <span className="text-sm font-semibold" style={{ color: '#fff' }}>{locale === 'zh' ? '亚马逊' : 'AWS Cloud'}</span>
                </div>
                <p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.5)' }}>
                  {locale === 'zh' ? '一键部署，7*24小时在线' : 'One-Click Deploy，7*24 online'}
                </p>
              </div>
              ...
              <div className="absolute top-0 left-8 px-2.5 py-1 rounded-full text-[10px] font-medium animate-bounce" style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#6ee7b7', border: '1px solid rgba(16, 185, 129, 0.3)', animationDuration: '3s' }}>Youtube</div>
      ...
      <div className="border-b border-gray-100 bg-white">
        <div className="max-w-5xl mx-auto px-4 py-4 flex flex-wrap items-center gap-2 text-xs text-gray-400">
          ...
          {[
            ...
            { name: locale === 'zh' ? '亚马逊' : 'AWS Cloud', color: '#0052d9' },
            { name: 'DigitalOcean', color: '#0069ff' },
            { name: 'Hostinger', color: '#6c47ff' },
            { name: 'Meta', color: '#0f62fe' },
            { name: 'Codecademy', color: '#1557ff' },
            { name: locale === 'zh' ? 'Youtube' : 'Youtube', color: '#fb7299' },
            { name: 'MiniMax', color: '#7c3aed' },
            { name: locale === 'zh' ? 'Github' : 'Github', color: '#3b82f6' },
      ...
      <footer className="hero-glow py-8 mt-4">
        <div className="max-w-5xl mx-auto px-4 text-center text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>
          {t.footerMadeBy} <span style={{ color: 'rgba(255,255,255,0.7)' }}>Terry</span> · <a href={homeUrl} className="hover:underline" style={{ color: 'rgba(255,255,255,0.6)' }}>OpenClaw Learn Hub</a> · <a href="https://www.clawlearnhub.com" target="_blank" rel="noopener noreferrer" className="hover:underline" style={{ color: 'rgba(255,255,255,0.6)' }}>GitHub</a>
        </div>
      </footer>
